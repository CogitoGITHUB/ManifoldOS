...
(define (provenance-file config-file)
  "Return a 'provenance' file describing CONFIG-FILE, a <local-file> or #f."
...
(define (provenance-entry config-file)
  "Return system entries describing the operating system configuration file."
  (mbegin %store-monad
...
itself: the channels used when building the system (always empty), and its
configuration file, when available.")))

(define (sexp->system-provenance sexp)
  "Parse SEXP from /run/current-system/provenance and return two values:
the list of channels (always empty) and the OS configuration file or #f."
  (match sexp
...
(define (system-provenance system)
  "Given SYSTEM, a system generation file name, return two values: the
list of channels (always empty) and its configuration file."
  (catch 'system-error
    (lambda ()
      (sexp->system-provenance
